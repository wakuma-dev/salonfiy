...
export default function HowItWorks(){
  ...
  return(
      <section id="how-it-works" className={`w-full h-auto ${theme === "light" ? "bg-[#f7f7f7] text-black" : "bg-[#121212] text-white"}`}>
        <div className="max-w-9/10 mx-auto flex flex-col items-center justify-center text-center gap-2 py-4">
            ...
            <p className="text-[18px] leading-[28px] font-normal text-[#64748b]">Book your next beauty appointment in 3 simple steps</p>
                ...
                {items.map((item) => {
                    ...
                    return(
                        <div key={item.title} className={`flex flex-col items-start text-left gap-2 p-6
                             rounded-xl w-full border border-black ${theme === "light" ? "bg-white" : "bg-[#1e1e1e]"}`}>
                         ...
                         <p className="text-[14px] leading-[18px] font-normal text-[#64748b]">{item.desc}</p>
                        </div>
                    )
                })}
            </div>
